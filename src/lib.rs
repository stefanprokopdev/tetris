...
use wasm_react::hooks::{use_callback, use_effect, use_js_ref, Deps};
use wasm_react::props::Style;
use wasm_react::{c, export_components, h, hooks::use_state, Component};
use web_sys::{window, Element, HtmlElement, KeyboardEvent};

use crate::tetris::{Direction, Tetris};
...
impl Component for App {
    fn render(&self) -> wasm_react::VNode {
        ...
        h!(div)
            ...
            .build(c![..tetris.value().iter_positions().map(|position| {
                let typ = tetris.value().get(position);
                h!(div)
